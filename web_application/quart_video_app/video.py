...
async def auto_video():
    ...
    return await send_file(os.path.join(os.path.dirname(__file__), "video.mp4"), conditional=True)


@app.route("/chunked_video.mp4")
async def chunked_video():
    # Force the response to be chunked in a 100_000 bytes max size.
    response = await send_file(os.path.join(os.path.dirname(__file__), "video.mp4"))
